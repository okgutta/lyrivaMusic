...
const getAudioAnalysisForTrack = async (uri: string): Promise<AudioAnalysisData | null> => {
  ...
  const request = getDynamicAudioAnalysis(uri)
    .then((analysis) => {
      // 只缓存成功结果。null 可能是"确认无分析"（404，持久缓存 notFound
      // 会拦住）或临时失败（429/网络，audioAnalysis.ts 有 60s retryAfter 冷却，
      // 冷落后会重试）——把 null 也缓存进内存会让临时失败在整首歌期间
      // 永不重试，动态背景停摆到切歌。
      if (analysis) {
...
const setDynamicBackgroundAnimationSpeed = (speed: number) => {
  KawarpMap.forEach((kawarpInstance) => {
    try {
      kawarpInstance.setOptions({ animationSpeed: speed });
    } catch (error) {
      dynamicBgLogger.debug("Failed to update background animation speed", error);
    }
  });
};
...
// 清理 NPV（正在播放侧栏）动态背景：切到静态背景模式时旧动态背景应移除
...
const handlePlaybackProgress = async (): Promise<void> => {
  ...
  KawarpMap.forEach((kawarpInstance) => {
    try {
      kawarpInstance.setOptions({ animationSpeed: speedMultiplier });
    } catch (error) {
      dynamicBgLogger.debug("Failed to apply analysis animation speed", error);
    }
  });
};

Global.Event.listen("playback:progress", () => {
  void handlePlaybackProgress().catch((error) => {
    dynamicBgLogger.error("Failed to handle playback progress", error);
  });
});
