...
async function main() {
  ...
  // Keep settings reachable even if a later player integration fails to start.
  ...
  // Spicetify initializes Keyboard asynchronously after the other APIs.
  // Platform.OnSpotifyReady (and the extension loader) does not wait for it.
  Whentil.When(
    () => typeof Spicetify.Keyboard?.registerImportantShortcut === "function",
    () => {
...
function registerSettingsMenu() {
  ...
  // Spicetify 2.45 filters profile items by the old userWidget classes. Spotify
  // 1.3 uses an aria-expanded avatar button instead, and may strip data-testid.
  const profileMenuItem = settingsMenuItem as Spicetify.Menu.Item & {
    shouldAdd?: (props: unknown, trigger: string, target: Element) => boolean;
  };
  const originalShouldAdd = profileMenuItem.shouldAdd;
  if (originalShouldAdd) {
    profileMenuItem.shouldAdd = (props, trigger, target) => {
      if (!(target instanceof Element)) return false;
      if (originalShouldAdd.call(profileMenuItem, props, trigger, target)) return true;
      if (trigger !== "click") return false;
      if (target.closest('[data-testid="user-widget-link"]')) return true;

      const button = target.closest("button[aria-expanded][aria-label]");
      return (
        !!button?.closest(".Root__globalNav .main-actionButtons") &&
        // Notifications/friends live in a separate action group. Onboarding
        // uses the controlled trigger, so neither receives profile settings.
        !button.closest(".main-actionButtons-spacer")
      );
    };
  }
