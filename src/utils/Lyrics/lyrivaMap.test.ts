...
import { buildLyrivaModel, mapTranslations, metaToLevel, confidenceOf } from "./lyrivaMap.ts";
...
// ── Line 模型：syncedLyrics 充足 → 时间换算（ms→秒）+ EndTime 链 ─────────────
{
  ...
  check("StartTime 秒换算", (model?.Content?.[0]?.StartTime ?? 0) === 5, model?.Content?.[0]?.StartTime);
  check("EndTime 取下一行", (model?.Content?.[0]?.EndTime ?? 0) === 12, model?.Content?.[0]?.EndTime);
  check("末行 EndTime +4s", (model?.Content?.[2]?.EndTime ?? 0) === 23, model?.Content?.[2]?.EndTime);
...
// ── metaToLevel / confidenceOf 兜底 ─────────────────────────────────────────
{
  ...
  check("缓存缺失 matchLevel → qualityScore>=90 判 HIGH", metaToLevel({ qualityScore: 100 }) === "HIGH");
  check("缓存缺失 matchLevel + qualityScore<90 → GOOD", metaToLevel({ qualityScore: 80 }) === "GOOD");
  // matchScore 单位不稳定：0-1 直通；否则用 qualityScore/100
  check("matchScore 0-1 直通", confidenceOf({ matchScore: 0.98 }) === 0.98);
  check("matchScore 非 0-1 → qualityScore/100", confidenceOf({ matchScore: 7, qualityScore: 100 }) === 1);
  check("无 match 信息 → 1", confidenceOf({}) === 1);
}
...
// ── translation LRC 对齐 ─────────────────────────────────────────────────────
{
  ...
  const model = buildLyrivaModel(data, target);
  check("translation LRC 对齐", model?.Content?.[1]?.Translation === "二", model?.Content?.[1]?.Translation);
}
...
// ── mapTranslations：纯文本按行序兜底 ────────────────────────────────────────
{
  ...
  check("空输入返回空串数组", mapTranslations(null, [1, 2])[0] === "", mapTranslations(null, [1, 2]));
}
...
// ── 无词：两者都不足 → null ──────────────────────────────────────────────────
{
  const data = { track: { title: "x" }, plainLyrics: "", syncedLyrics: [{ startMs: 0, text: "a" }] };
