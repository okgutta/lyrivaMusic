...
// 每次调用时求值：简单歌词模式可随时切换，模块级 const 会在切换后冻结旧偏移
function getSubstractions() {
  return {
...
export default function Emphasize(
  ...
  const Substractions = getSubstractions();
  ...
  const currentLine = LyricsObject.Types.Syllable.Lines?.[CurrentLineLyricsObject];
  if (CurrentLineLyricsObject >= 0 && currentLine?.Syllables) {
    currentLine.Syllables.Lead.push({
      HTMLElement: word,
      StartTime: StartTime,
      EndTime: EndTime,
