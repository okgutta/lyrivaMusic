...
export function ApplySyllableLyrics(data: LyricsData, UseRomanized: boolean = false): void {
  ...
  content.forEach((line, index, arr) => {
    ...
    // Lead 可选：缺失时按 0 处理（与管线其他环节的防御式访问一致）
    const lead = line.Lead;
    const leadStart = lead?.StartTime ?? 0;
    const leadEnd = lead?.EndTime ?? 0;
    ...
    LyricsObject.Types.Syllable.Lines.push({
      ...
      StartTime: ConvertTime(leadStart),
      EndTime: ConvertTime(lineEndTime),
      TotalTime: ConvertTime(lineEndTime) - ConvertTime(leadStart),
    });
    ...
    if (
      ...
      const nextLeadStart = arr[index + 1].Lead.StartTime;
      LyricsObject.Types.Syllable.Lines.push({
        HTMLElement: musicalLine,
      ...
      const gapStartTime = ConvertTime(leadEnd);
      const totalTime = ConvertTime(nextLeadStart) - gapStartTime;
      const baseDotTime = totalTime / 3;
      const dotPadding = getInterludeTimePadding() / 3;
