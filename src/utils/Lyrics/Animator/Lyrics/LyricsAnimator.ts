...
const createDotSprings = () => {
  if ($simpleLyricsMode.get()) {
    return {
      Scale: {
        Step: () => DotScaleSpline.at(0),
        SetGoal: () => {},
      },
      YOffset: {
        Step: () => DotYOffsetSpline.at(0),
        SetGoal: () => {},
      },
      Glow: {
        Step: () => DotGlowSpline.at(0),
        SetGoal: () => {},
      },
      Opacity: new Spring(
...
function resetSyllableLine(line: LyricsSyllable): void {
                    ...
                    if (nextWord && !nextWord?.LetterGroup) {
                      if (!nextWord.PreSLMAnimated) {
                        ...
                        setTimeout(
                          ...
                          Math.max(0, totalDuration * 0.845 - 130)
                        );
                      }
                    }
                  }
                    ...
                    if (nextWord) {
                      if (!nextWord.PreSLMAnimated) {
                        ...
                        setTimeout(
                          ...
                          Math.max(0, totalDuration * 0.6 - 22)
                        );
                      }
                    }
                  }
