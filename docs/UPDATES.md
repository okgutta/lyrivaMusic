...
| 文件                     | 用途                                         |
| ------------------------ | -------------------------------------------- |
| `lyrivamusic.js`         | 用户首次安装的加载器，含离线兜底版本         |
| `lyrivamusic-runtime.js` | 被加载器下载的完整扩展，含样式               |
| `manifest.json`          | 版本、加载器协议、运行包地址、大小和 SHA-256 |
| `SHA256SUMS.txt`         | 发布附件校验                                 |

发布任务先上传 Release 草稿附件，再在 `updates` 分支的一次提交中写入运行包与清单；验证分发地址后才公开 Release。已发布版本的分发文件不可覆盖。
